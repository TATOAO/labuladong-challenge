...
class Solution:
    ...
    def isBipartite(self, graph: List[List[int]]) -> bool:
        ...
        return self.subBipartite(A_set, B_set, graph, 0)

        
    def subBipartite(self, A_set, B_set, graph: List[List[int]], ith_graph) -> bool:

        A_set = set([ith_graph])
        B_set = set([] + graph[ith_graph])
        N = len(graph)

        for i in range(ith_graph + 1, N, 1):

            neibour_set = set(graph[i])
            A_intersect = A_set.intersection(neibour_set)
            B_intersect = B_set.intersection(neibour_set)

            i_in_A = i in A_set
            i_in_B = i in B_set
            
            if i_in_A and i_in_B:
                return False

            elif i_in_A:
                # neibours should all in B
                if len(A_intersect) > 0:
                    return False

                B_set = B_set.union(neibour_set)
                continue

            elif i_in_B:
                # neibours should all in A
                if len(B_intersect) > 0:
                    return False
                A_set = A_set.union(neibour_set)
                continue

            else:
                # i not in A and not in B

                if len(A_intersect) > 0 and \
                        len(B_intersect) > 0:
                    return False
                
                elif len(B_intersect) > 0 and len(A_intersect) == 0:

                    # the i must be in A set
                    A_set.add(i)
                    B_intersect = B_intersect.union(neibour_set)

                elif len(A_intersect) > 0 and len(B_intersect) == 0:

                    # the i must be in B set
                    B_set.add(i)
                    A_intersect = A_intersect.union(neibour_set)
                ...
                if len(A_intersect) == 0 and \
                    ...
                    if_put_A = self.subBipartite(A_set, B_set, graph, i)
                    if if_put_A:
                        return True
                    if_put_B = self.subBipartite(B_set, A_set, graph, i)
                    return if_put_B
